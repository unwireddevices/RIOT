...
static void pm_select_run_mode(uint8_t pm_mode) {
    switch(pm_mode) {
        case PM_ON:
            ...
            puts("Switching to PM_ON");
            clk_init();
            break;
        case PM_IDLE:
            DEBUG("Switching to PM_IDLE");
            puts("Switching to PM_IDLE");
            break;
        default:
            DEBUG("Switching to PM_IDLE");
            puts("Switching to PM_IDLE");
...
{
    pm_run_mode = PM_ON;
    puts("Initialize pm_mode to PM_ON");
...
enum pm_mode pm_set(enum pm_mode target)
{
    switch (target) {
        ...
        case PM_POWERDOWN:         /* STOP mode */
            ...
            puts("Switching to PM_POWERDOWN");
            /* Clear Wakeup flag */    
            PWR->CR |= PWR_CR_CWUF;
