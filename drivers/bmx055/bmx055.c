...
int bmx055_init(bmx055_t *dev, const bmx055_params_t *params)
{
     ...
     */
    if (i2c_write_reg(BUS, ADDR_MAG, REG_MAG_PWRCTRL, BIT_MAG_PWRCTRL_VAL, 0) != 1) {
        DEBUG("[bmx055] error: no connection to magnetometer\n");
        return BMX055_NODEV;
    }
    if (i2c_read_reg(BUS, ADDR_MAG, REG_MAG_CHIPID, &tmp, 0) != 1) {
        DEBUG("[bmx055] error: no connection to magnetometer\n");
        return BMX055_NODEV;
    }
    ...
    if (i2c_read_reg(BUS, ADDR_ACC, REG_ACC_CHIPID, &tmp, 0) != 1) {
    ...
    if (i2c_read_reg(BUS, ADDR_GYRO, REG_GYRO_CHIPID, &tmp, 0) != 1) {
     ...
     */
    if (i2c_write_reg(BUS, ADDR_MAG, REG_MAG_OPMODE, (dev->p.mag_rate << 3), 0) != 1) {
     ...
     */
    if (i2c_write_reg(BUS, ADDR_ACC, 0x14, 0xB6, 0) != 1) {
        DEBUG("[bmx055] erro: setting accelerometer opmode\n");
        return BMX055_NOWRITE;
    }

    /* setting acc range */
    if (i2c_write_reg(BUS, ADDR_ACC, REG_ACC_RANGE, acc_ranges[dev->p.acc_range], 0) != 1) {
        DEBUG("[bmx055] error: setting accelerometer range\n");
        return BMX055_NOWRITE;
    }

    /* enable acc shadowing */
    if (i2c_write_reg(BUS, ADDR_ACC, REG_ACC_SHDW, REG_ACC_SHDW_ENABLE, 0) != 1) {
     ...
     */
    if (i2c_write_reg(BUS, ADDR_GYRO, REG_GYRO_PWRMD, REG_GYRO_PWRMD_NORM, 0) != 1) {
        DEBUG("[bmx055] error: setting gyroscope opmode\n");
        return BMX055_NOWRITE;
    }

    /* setting gyro scale */
    if (i2c_write_reg(BUS, ADDR_GYRO, REG_GYRO_SCALE, dev->p.gyro_scale, 0) != 1) {
        DEBUG("[bmx055] error: setting gyroscope scale\n");
        return BMX055_NOWRITE;
    }

    /* enable gyro shadowing */
    if (i2c_write_reg(BUS, ADDR_GYRO, REG_GYRO_SHDW, REG_GYRO_SHDW_EN, 0) != 1) {
...
int bmx055_mag_read(const bmx055_t *dev, int16_t *data)
{
    ...
    if (i2c_read_regs(BUS, ADDR_MAG, REG_MAG_DATA, &tmp, 7, 0) != 7) {
...
int bmx055_acc_read(const bmx055_t *dev, int16_t *data)
{
    ...
    if (i2c_read_regs(BUS, ADDR_ACC, REG_ACC_DATA, &tmp, 7, 0) != 7) {
