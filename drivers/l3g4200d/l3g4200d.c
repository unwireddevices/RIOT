                  ...
                  l3g4200d_mode_t mode, l3g4200d_scale_t scale)
{
    ...
    if (i2c_write_reg(dev->i2c, dev->addr, L3G4200D_REG_CTRL1, tmp, 0) != 1) {
        i2c_release(dev->i2c);
        return -1;
    }
    tmp = ((scale & 0x3) << L3G4200D_CTRL4_FS_POS) | L3G4200D_CTRL4_BDU;
    if (i2c_write_reg(dev->i2c, dev->addr, L3G4200D_REG_CTRL4, tmp, 0) != 1) {
        i2c_release(dev->i2c);
        return -1;
    }
    i2c_release(dev->i2c);
...
int l3g4200d_read(const l3g4200d_t *dev, l3g4200d_data_t *data)
{
    ...
    i2c_read_regs(dev->i2c, dev->addr, L3G4200D_REG_OUT_X_L | L3G4200D_AUTOINC, tmp, 6, 0);
    i2c_release(dev->i2c);
...
int l3g4200d_enable(const l3g4200d_t *dev)
{
    ...
    i2c_acquire(dev->i2c);
    res = i2c_read_reg(dev->i2c, dev->addr, L3G4200D_REG_CTRL1, &tmp, 0);
    if (res < 1) {
        i2c_release(dev->i2c);
        return res;
    }
