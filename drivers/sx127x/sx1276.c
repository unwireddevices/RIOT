...
sx1276_dio_irq_handler *dio_irq[] = { sx1276_on_dio_0_irq, sx1276_on_dio_1_irq,
                            sx1276_on_dio_2_irq, sx1276_on_dio_3_irq,
                            sx1276_on_dio_4_irq, NULL };
...
void sx1276_init( sx1276_t* dev, sx_1276_events_t *events )
{
    ...
    for(i = 0; i < sizeof(radio_regs_init) / sizeof(sx1276_radio_registers_t); i++)
    {
        sx1276_set_modem(dev, radio_regs_init[i].modem);
        sx1276_reg_write(dev, radio_regs_init[i].addr, radio_regs_init[i].value);
    }
...
void sx1276_set_modem(sx1276_t *dev, sx1276_radio_modems_t modem )
{
    ...
    case MODEM_FSK:
        sx1276_set_op_mode(dev, RF_OPMODE_SLEEP );
        sx1276_reg_write(dev, REG_OPMODE, (sx1276_reg_read(dev, REG_OPMODE) & RFLR_OPMODE_LONGRANGEMODE_MASK ) | RFLR_OPMODE_LONGRANGEMODE_OFF);
    ...
    case MODEM_LORA:
        sx1276_set_op_mode(dev, RF_OPMODE_SLEEP);
        sx1276_reg_write(REG_OPMODE, (sx1276_reg_read(dev, REG_OPMODE) & RFLR_OPMODE_LONGRANGEMODE_MASK) | RFLR_OPMODE_LONGRANGEMODE_ON);
