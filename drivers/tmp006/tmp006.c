...
int tmp006_init(tmp006_t *dev, const tmp006_params_t *params)
{
    ...
    i2c_init(BUS);
    /* test device id */
    if (i2c_read_regs(BUS, ADDR, TMP006_REGS_DEVICE_ID, reg, 2, 0) != 2) {
        i2c_release(BUS);
    ...
    reg[0] = (tmp >> 8);
    reg[1] = tmp;
    if (i2c_write_regs(BUS, ADDR, TMP006_REGS_CONFIG, reg, 2, 0) != 2) {
        i2c_release(BUS);
...
int tmp006_set_active(const tmp006_t *dev)
{
    ...
    i2c_acquire(BUS);
    if (i2c_read_regs(BUS, ADDR, TMP006_REGS_CONFIG, reg, 2, 0) != 2) {
        i2c_release(BUS);
        return -TMP006_ERROR_BUS;
    }
...
int tmp006_read(const tmp006_t *dev, int16_t *rawv, int16_t *rawt, uint8_t *drdy)
{
    ...
    *drdy = reg[1] & (TMP006_CONFIG_DRDY);
    ...
    i2c_acquire(BUS);
    if (i2c_read_regs(BUS, ADDR, TMP006_REGS_V_OBJECT, reg, 2, 0) != 2) {
        i2c_release(BUS);
        return -TMP006_ERROR_BUS;
    }
    i2c_release(BUS);
    ...
    *rawv = ((uint16_t)reg[0] << 8) | reg[1];
    ...
    i2c_acquire(BUS);
    if (i2c_read_regs(BUS, ADDR, TMP006_REGS_T_AMBIENT, reg, 2, 0) != 2) {
        i2c_release(BUS);
        return -TMP006_ERROR_BUS;
    }
    i2c_release(BUS);
