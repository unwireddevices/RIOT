...
static shell_command_t shell_commands_gate[10] = {};

static void init_role(config_role_t role) {
	switch (role) {
        case ROLE_NORMAL:
            init_gate((shell_command_t **) &shell_commands_gate);
            break;

        case ROLE_NO_EUI64:
            init_no_eui64((shell_command_t **) &shell_commands_gate);
            break;

        default:
        case ROLE_NO_CFG:
            init_no_cfg((shell_command_t **) &shell_commands_gate);
...
int main(void)
{
    ...
    shell_run(shell_commands_gate, line_buf, SHELL_DEFAULT_BUFSIZE);
