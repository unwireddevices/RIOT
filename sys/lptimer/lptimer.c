 ...
 * @{
 * @file
 * @brief lptimer convenience functionality
...
void _lptimer_tsleep(uint32_t offset, uint32_t long_offset)
{
    ...
    mutex_lock(&mutex);
    _lptimer_set64(&timer, offset, long_offset);
    mutex_lock(&mutex);
}
...
void _lptimer_periodic_wakeup(uint32_t *last_wakeup, uint32_t period) {
     ...
     * _lptimer_periodic_sleep returns.
     * If this happens, last_wakeup may specify a time in the future when the
     * next call to _lptimer_periodic_sleep is made, which in turn will trigger
    ...
    if (offset < LPTIMER_PERIODIC_SPIN) {
        _lptimer_spin(offset);
    }
    else {
        if (offset < LPTIMER_PERIODIC_RELATIVE) {
            /* NB: This will overshoot the target by the amount of time it took
             * to get here from the beginning of lptimer_periodic_wakeup()
             *
             * Since interrupts are normally enabled inside this function, this time may
             * be undeterministic. */
            target = _lptimer_now() + offset;
        }
        mutex_lock(&mutex);
...
static inline void _setup_msg(lptimer_t *timer, msg_t *msg, kernel_pid_t target_pid)
{
 ...
 * Additionally, the lptimer_t struct gets initialized.
 */
static void _setup_timer_msg(msg_t *m, lptimer_t *t)
{
    m->type = MSG_LPTIMER;
    m->content.ptr = m;
...
static int _msg_wait(msg_t *m, msg_t *tmsg, lptimer_t *t)
{
    msg_receive(m);
    if (m->type == MSG_LPTIMER && m->content.ptr == tmsg) {
        /* we hit the timeout */
        return -1;
    }
    else {
...
int _lptimer_msg_receive_timeout64(msg_t *m, uint64_t timeout_ticks) {
    msg_t tmsg;
    lptimer_t t;
    _setup_timer_msg(&tmsg, &t);
    _lptimer_set_msg64(&t, timeout_ticks, &tmsg, sched_active_pid);
    return _msg_wait(m, &tmsg, &t);
}

int _lptimer_msg_receive_timeout(msg_t *msg, uint32_t timeout_ticks)
{
    msg_t tmsg;
    lptimer_t t;
    _setup_timer_msg(&tmsg, &t);
    _lptimer_set_msg(&t, timeout_ticks, &tmsg, sched_active_pid);
...
int lptimer_mutex_lock_timeout(mutex_t *mutex, uint64_t timeout)
{
    lptimer_t t;
    ...
    if (timeout != 0) {
        ...
        _lptimer_set64(&t, timeout, timeout >> 32);
    }
