...
static int _get(gnrc_netif_t *netif, gnrc_netapi_opt_t *opt)
{
    ...
    switch (opt->opt) {
        ...
        case NETOPT_LORAWAN_DEVICE_CLASS:
            assert(opt->data_len == sizeof(loramac_class_t));
            *((loramac_class_t *) opt->data) = netif->lorawan.mac.device_class;
            break;
...
static int _set(gnrc_netif_t *netif, const gnrc_netapi_opt_t *opt)
{
    ...
    switch (opt->opt) {
        ...
        case NETOPT_LORAWAN_DEVICE_CLASS:
            assert(opt->data_len == sizeof(loramac_class_t));
            netif->lorawan.mac.device_class = *((loramac_class_t *) opt->data);
            break;
        case NETOPT_LORAWAN_TX_POWER:
            assert(opt->data_len == sizeof(uint8_t));
