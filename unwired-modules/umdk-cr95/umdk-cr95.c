...
static umdk_st95_config_t umdk_st95_config = { ST95_IFACE_UART, ST95_READER, ISO14443A_SELECT };
			...
			case UMDK_ST95_MSG_PROTOCOL: {
				...
				if(_check_pack(num_bytes_rx)) {				
					if((rxbuf[0] == 0x00) && (rxbuf[1] == 0x00)) {
						current_state = UMDK_ST95_PACK_OK;
						if(umdk_st95_config.mode == ST95_READER) {
							msg_rx.type = UMDK_ST95_MSG_UID;
							_cmd_send_receive(send_1a, 2);
						}
						else if(umdk_st95_config.mode == ST95_WRITER) {
...
static void reset_config(void) {
	...
	umdk_st95_config.mode = ST95_READER;
...
void umdk_st95_init(uint32_t *non_gpio_pin_map, uwnds_cb_t *event_callback)
{
	...
	if(umdk_st95_config.mode == ST95_WRITER) {
		puts("[umdk-" _UMDK_NAME_ "]: Writer mode");
	}
	else if(umdk_st95_config.mode == ST95_READER) {
...
{	
	...
	if(cmd->data[0] == 0x00) {	// Send data
		...
		if(cmd->data[1] == ST95_WRITER) {
			umdk_st95_config.mode = ST95_WRITER;
			puts("WRITER mode");
		}
		else if(cmd->data[1] == ST95_READER) {
			umdk_st95_config.iface = ST95_READER;
			puts("READER mode");
		}
				save_config();
	}
